...
class Player {
    ...
    canAnkanTiles(drawTile) {
        let count = 0, isFourTile = false;
        for (let i = 1; i < this.tiles.length; i++) {
            if (this.tiles[i].isEqual(drawTile)) {
                count += 1;
                if (count === 4) {
                    isFourTile = true;
                }
        ...
        if (this.riichiTurn === -1 || !isFourTile) {
            return [];
        } // 4枚無い場合はカンできない
        const tileIdx = this.tiles.findIndex(tile => tile.isEqual(drawTile));
        this.tiles.splice(tileIdx, 1); // 手牌を13枚にする
        const myTilesCopy = this.tiles.slice();
        const ankoTiles = [];
        const souzuAnkoTiles = [];
        let kotzTiles = [];
        ...
        for (let i = 0; i < patterns.length; i++) {
            ...
            if (jantou.length === 0) {
                ...
                for (let j = 0; j < jantouCandidate.length; j++) {
                    ...
                    if ((shuntzTiles.length + kotzTiles.length + this.ankans.length === 4 && jantou.length === 0) ||
                        (shuntzTiles.length + kotzTiles.length + this.ankans.length === 3 && jantou.length === 1)) {
                        winnings.push(new Winning_1.Winning(kotzTiles, this.minkos, shuntzTiles, this.ankans, this.minkans, jantou, [], [], new Tile_1.Tile("", 0, ""), "draw", this.riichiTurn, this.turn));
                    }
                    this.tiles = myTilesCopy3.slice();
                    kotzTiles = kotzTilesCopy2.slice();
                    shuntzTiles = [];
                ...
                if ((shuntzTiles.length + kotzTiles.length + this.ankans.length === 4 && jantou.length === 0) ||
                    (shuntzTiles.length + kotzTiles.length + this.ankans.length === 3 && jantou.length === 2)) {
                    winnings.push(new Winning_1.Winning(kotzTiles, this.minkos, shuntzTiles, this.ankans, this.minkans, jantou, [], [], new Tile_1.Tile("", 0, ""), "draw", this.riichiTurn, this.turn));
                }
                this.tiles = myTilesCopy2.slice();
                kotzTiles = kotzTilesCopy.slice();
